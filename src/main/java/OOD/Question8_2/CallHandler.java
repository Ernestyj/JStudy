...
public class CallHandler {
     ...
     */
    List<List<OOD.Question8_2.Employee>> employeeLevels;

	/* queues for each call�s rank */
    List<List<Call>> callQueues; 

    protected CallHandler() {
    	employeeLevels = new ArrayList<List<OOD.Question8_2.Employee>>(LEVELS);
    	callQueues = new ArrayList<List<Call>>(LEVELS); 
    	
        // Create respondents.
        ArrayList<OOD.Question8_2.Employee> respondents = new ArrayList<OOD.Question8_2.Employee>(NUM_RESPONDENTS);
        ...
        // Create managers.
        ArrayList<OOD.Question8_2.Employee> managers = new ArrayList<OOD.Question8_2.Employee>(NUM_MANAGERS);
        managers.add(new OOD.Question8_2.Manager());
        employeeLevels.add(managers);

        // Create directors.
        ArrayList<OOD.Question8_2.Employee> directors = new ArrayList<OOD.Question8_2.Employee>(NUM_DIRECTORS);
        directors.add(new OOD.Question8_2.Director());
    ...
    public OOD.Question8_2.Employee getHandlerForCall(Call call) {
        for (int level = call.getRank().getValue(); level < LEVELS - 1; level++) {
            List<OOD.Question8_2.Employee> employeeLevel = employeeLevels.get(level);
            for (OOD.Question8_2.Employee emp : employeeLevel) {
                if (emp.isFree()) {
                    return emp;
                }
            }
    ...
    public void dispatchCall(Call call) {
        ...
        OOD.Question8_2.Employee emp = getHandlerForCall(call);
        if (emp != null) {
    ...
    public boolean assignCall(OOD.Question8_2.Employee emp) {
